...
const ARTBOARD = 22;
/** Extra canvas pad so Skia blur isn’t clipped. */
const GLOW_PAD_VB = 10;

const GLOW_LAYERS = [
  { color: GLOW_COLOR, opacity: 0.45, blur: 3.2 },
  { color: GLOW_COLOR, opacity: 0.7, blur: 2 },
  { color: GLOW_BRIGHT, opacity: 0.95, blur: 1.1 },
] as const;

export const GoldenTickIcon = ({ size = 22 }: { size?: number }) => {
  const padPx = (size / ARTBOARD) * GLOW_PAD_VB;
  const canvasSize = size + padPx * 2;
  const scale = size / ARTBOARD;

  const tickPath = useMemo(() => {
    const path = Skia.Path.MakeFromSVGString(TICK_PATH);
    if (!path) return null;
    path.transform(Skia.Matrix().scale(scale, scale));
    path.transform(Skia.Matrix().translate(padPx, padPx));
    return path;
  }, [padPx, scale]);

  if (!tickPath) return null;
    ...
    <View style={[styles.wrap, { width: size, height: size }]}>
      ...
      >
        {GLOW_LAYERS.map((layer) => (
          <SkiaPath
            key={`tick-glow-${layer.blur}`}
            path={tickPath}
            color={layer.color}
            opacity={layer.opacity}
            style="fill"
          >
            <Blur blur={layer.blur * scale} mode="decal" />
          </SkiaPath>
        ))}
        <SkiaPath path={tickPath} color="#FFFFFF" style="fill" />
      </Canvas>
    </View>
  );
};
