...
export function MemorisationSurahSelectionStep({
  goals,
  selectedSurahId,
  onSelectSurah,
  styles,
  ...
  return (
    <View style={{ gap: 10 }}>
      <Text style={styles.recitationMaxLabel}>
        {t("progressLogging.memorisationSelectSurah")}
      </Text>
      {availableGoals.map((goal) => {
        const isSelected = selectedSurahId === goal.id;
        return (
          <TouchableOpacity
            key={goal.id}
            onPress={() => onSelectSurah(goal.id)}
            activeOpacity={0.8}
            style={{
              flexDirection: "row",
              alignItems: "center",
