...
type Props = {
  ...
  onSelectHizb: (hizbId: string) => void;
  styles: any;
};

export function MemorisationHizbSelectionStep({
  goals,
  selectedHizbId,
  onSelectHizb,
  styles,
  ...
  return (
    <View style={{ gap: 10 }}>
      <Text style={styles.recitationMaxLabel}>
        {t("progressLogging.memorisationSelectHizb")}
      </Text>
      {availableGoals.map((goal) => {
        const isSelected = selectedHizbId === goal.id;
        return (
          <TouchableOpacity
            key={goal.id}
            onPress={() => onSelectHizb(goal.id)}
            activeOpacity={0.8}
            style={{
              flexDirection: "row",
              alignItems: "center",
