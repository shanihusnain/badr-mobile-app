...
import { useLogTawbahPrayerGoal } from "@/src/api/mutations/useLogTawbahPrayerGoal";
...
type TawbahPrayerStepId =
  | "date"
  | "prayers-quantity"
  | "start-time"
  | "time-spent";
const STEPS: TawbahPrayerStepId[] = [
  "date",
  "prayers-quantity",
  "start-time",
  "time-spent",
...
export default function TawbahPrayerLoggingFlow({
  ...
  const { mutateAsync: logTawbah, isPending: isLogging } = useLogTawbahPrayerGoal();
  ...
  const prayerType = resolvePrayerTypeFromGoalId(goalData.id) ?? "TAWBAH";
  ...
  const getStepHeader = (step: TawbahPrayerStepId) => {
    switch (step) {
      case "date":
        return {
  ...
  const renderStepContent = (step: TawbahPrayerStepId) => {
    switch (step) {
      case "date":
        return (
          <DateStep
