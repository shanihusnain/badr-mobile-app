...
type IstikharaStepId = "date" | "prayers-quantity" | "start-time" | "time-spent";
const STEPS: IstikharaStepId[] = ["date", "prayers-quantity", "start-time", "time-spent"];

type Props = { goalData: GoalData; onLogComplete?: (entry: ProgressLogEntry) => void; };
type FlowMode = "collapsed" | "active";
const toDateString = (date: Date) => moment(date).format("YYYY-MM-DD");

export default function IstikharaPrayerLoggingFlow({ goalData, onLogComplete }: Props) {
  ...
  const mockTitle = `${totalPrayersRequired} 2-Rak'ah Istikhara Prayers`;
  ...
  const resetFlow = useCallback(() => {
    ...
    setPrayersCount("1"); setStartHour("06"); setStartMinute("15"); setStartPeriod("am");
    setDurationHours("0"); setDurationMinutes("10"); setIsPeriodDropdownOpen(false);
  }, []);

  const handleConfirm = () => {
    setHasLogged(true);
    onLogComplete?.({ type: "istikhara-prayer", goalId: goalData.id, date: selectedDate, prayersCount, startTime: `${startHour}:${startMinute} ${startPeriod}`, durationHours, durationMinutes } as any);
    resetFlow();
  };
  ...
  const getStepHeader = (step: IstikharaStepId) => {
    switch (step) {
  ...
  const renderStepContent = (step: IstikharaStepId) => {
    switch (step) {
