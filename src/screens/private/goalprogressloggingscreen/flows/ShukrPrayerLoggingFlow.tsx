...
type ShukrStepId = "date" | "prayers-quantity" | "start-time" | "time-spent";
const STEPS: ShukrStepId[] = ["date", "prayers-quantity", "start-time", "time-spent"];

type Props = { goalData: GoalData; onLogComplete?: (entry: ProgressLogEntry) => void; };
type FlowMode = "collapsed" | "active";
const toDateString = (date: Date) => moment(date).format("YYYY-MM-DD");

export default function ShukrPrayerLoggingFlow({ goalData, onLogComplete }: Props) {
  ...
  const mockTitle = `${totalPrayersRequired} 2-Rak'ah Shukr Prayers`;
  ...
  const resetFlow = useCallback(() => {
    ...
    setPrayersCount("2"); setStartHour("06"); setStartMinute("15"); setStartPeriod("am");
    setDurationHours("0"); setDurationMinutes("10"); setIsPeriodDropdownOpen(false);
  }, []);

  const handleConfirm = () => {
    setHasLogged(true);
    onLogComplete?.({ type: "shukr-prayer", goalId: goalData.id, date: selectedDate, prayersCount, startTime: `${startHour}:${startMinute} ${startPeriod}`, durationHours, durationMinutes } as any);
    resetFlow();
  };
  ...
  const getStepHeader = (step: ShukrStepId) => {
    switch (step) {
      ...
      case "prayers-quantity": return { icon: <MaterialCommunityIcons name="heart" size={16} color={Colors.light.white} />, label: "How many 2-rak'ah prayers did you pray?" };
  ...
  const renderStepContent = (step: ShukrStepId) => {
    switch (step) {
          ...
          <View style={localStyles.summaryCard}>
            <View style={localStyles.summaryBody}>
              <View style={localStyles.summaryIconCircle}>
                <MaterialCommunityIcons name="heart" size={18} color={Colors.light.white} />
              </View>
