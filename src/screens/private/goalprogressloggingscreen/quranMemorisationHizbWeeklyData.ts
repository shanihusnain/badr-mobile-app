import { getHizbMemorisationLogsForFilter } from "./quranMemorisationHizbData";
import {
  getHizbMemorisationGoalsForFilter,
  type MemorisationHizbFilterId,
} from "./quranMemorisationHizbGoals";

export type HizbMemorisationDayProgress = {
  day: string;
  date: string;
  ayahsLogged: number;
  isLogged: boolean;
...
export type HizbMemorisationWeekSummary = {
  hizbFilter: MemorisationHizbFilterId;
  hizbName: string;
  weekDays: HizbMemorisationDayProgress[];
...
export type HizbMemorisationCycleSummary = {
  hizbFilter: MemorisationHizbFilterId;
  weeks: HizbMemorisationWeekSummary[];
...
function aggregateAyahsByDate(
  logs: ReturnType<typeof getHizbMemorisationLogsForFilter>,
): Map<string, number> {
...
function buildHizbProgress(hizbFilter: MemorisationHizbFilterId) {
  const goals = getHizbMemorisationGoalsForFilter(hizbFilter);
...
function buildWeekSummary(
  ...
  weekIndex: number,
  ayahsByDate: Map<string, number>,
  progress: ReturnType<typeof buildHizbProgress>,
): HizbMemorisationWeekSummary {
  const weekStart = addDays(CYCLE_START, weekIndex * 7);
  const weekEnd = addDays(weekStart, 6);
  ...
  return {
    ...
    motivationalQuoteKey: "progressLogging.quranMemorisationHizbWeekQuote",
  };
}

export function getQuranMemorisationHizbCycleSummary(
  hizbFilter: MemorisationHizbFilterId = "all",
): HizbMemorisationCycleSummary {
  const logs = getHizbMemorisationLogsForFilter(hizbFilter);
  const ayahsByDate = aggregateAyahsByDate(logs);
  const progress = buildHizbProgress(hizbFilter);

  const weeks = Array.from({ length: MEMORISATION_CYCLE_WEEKS }, (_, weekIndex) =>
    buildWeekSummary(hizbFilter, weekIndex, ayahsByDate, progress),
  );
...
export function clampHizbMemorisationWeekIndex(weekIndex: number): number {
