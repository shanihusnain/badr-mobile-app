...
export type LogTawbahPrayerPayload = {
  date: string;
  count: number;
...
const logTawbahPrayer = async (payload: LogTawbahPrayerPayload) => {
  const response = await api.post(
    "api/goal-cycles/current/prayer-goals/TAWBAH/log",
    payload,
  );
  return response.data;
};

export const useLogTawbahPrayerGoal = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: logTawbahPrayer,
    onSuccess: () => {
