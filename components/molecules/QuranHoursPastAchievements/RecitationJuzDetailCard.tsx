...
import type { JuzProgressRailRow } from "@/src/screens/private/goalprogressloggingscreen/quranRecitationJuzPastAchievementData";

type RecitationJuzDetailCardProps = {
  row: JuzProgressRailRow;
  analyticsView: JuzAnalyticsView;
  formatTimeChip: (minutes: number) => string;
};

export function RecitationJuzDetailCard({
  row,
  analyticsView,
  formatTimeChip,
}: RecitationJuzDetailCardProps) {
  ...
  const title =
    row.juzNumber === null
      ...
      : t("progressLogging.juzRowTitle", {
          number: formatNumber(row.juzNumber),
        });
    ...
    <View style={styles.card}>
      <View style={styles.metaRow}>
        <Text style={styles.metaTitle} numberOfLines={2}>
          {title}
          {row.rangeLabel ? (
            <Text style={styles.metaTitle}> | {row.rangeLabel}</Text>
          ) : null}
        </Text>
