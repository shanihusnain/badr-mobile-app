...
export function PrayerPastAchievements({ goalId, isDetailed = false }: Props) {
  ...
  const { data: achievementsApiData, isLoading: isAchievementsLoading } =
    useGetPrayerGoalAchievements(prayerType, {
      ...
      enabled: usesApiAchievements && !!prayerType,
    });
  const showPlaceholders =
    usesApiAchievements && (!achievementsApiData || isAchievementsLoading);
  ...
  const renderInsights = () => {
    if (!isDetailed && !usesApiAchievements) return null;

    const cards = usesApiAchievements
